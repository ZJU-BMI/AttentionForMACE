...
class ExperimentSetup(object): # 五折交叉验证
    kfold = 5
    test_size = 1 / kfold
...
def evaluate(tol_label, tol_pred, result_file='resources/save/evaluation_result.csv'):
    ...
    y_true = np.argmax(tol_label, axis=1) # 返回最大值的索引
    y_pred = np.argmax(tol_pred, axis=1)

    accuracy = accuracy_score(y_true, y_pred)  # 返回分类准确率
    auc = roc_auc_score(tol_label, tol_pred, average=None) # ROC曲线
    # fpr, tpr, thresholds = roc_curve(y_true, y_score)
    precision = precision_score(y_true, y_pred, average=None) # 精确值
    recall = recall_score(y_true, y_pred, average=None) # 召回率
    f_score = f1_score(y_true, y_pred, average=None) # f值
    with open(result_file, 'a', newline='') as csv_file:
        f_writer = csv.writer(csv_file, delimiter=',')
        for i in range(classes):
            y_score = tol_pred[:, i] # 读取预测值与真实值
            y_true = tol_label[:, i]
            fpr, tpr, thresholds = roc_curve(y_true, y_score)
...
def model_experiments(model, data_set, result_file):
    ...
    for train_idx, test_idx in split:
        ...
        train_set = DataSet(train_static, train_dynamic, train_y) # 包含多个data table，用dataset["表名"]可以得到
